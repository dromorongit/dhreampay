  ...
  if (batches.length === 0) {
    return (
      <div className="bg-white rounded-xl p-6 border border-[#e2e8f0]">
        <p className="text-[#475569] text-center py-8">No settlement batches uploaded yet. Upload your first file to get started.</p>
      </div>
    );
  }
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-[#1e3a5f]">
              <th className="text-left text-xs font-medium text-white pb-3 px-4">Batch ID</th>
              ...
              <th className="text-left text-xs font-medium text-white pb-3 px-4">File Name</th>
              <th className="text-right text-xs font-medium text-white pb-3 px-4">Total Rows</th>
              <th className="text-right text-xs font-medium text-white pb-3 px-4">Total Amount</th>
          ...
          <tbody>
            {batches.map((batch) => (
              <tr key={batch._id} className="border-b border-[#e2e8f0] last:border-0 hover:bg-[#f1f5f9] transition-colors">
                <td className="py-3 px-4 text-sm text-[#0f172a] font-mono truncate max-w-[120px]" title={batch.batchId}>
                  {batch.batchId}
                </td>
                <td className="py-3 px-4 text-sm text-[#475569]">{formatDate(batch.batchDate)}</td>
                ...
                <td className="py-3 px-4 text-sm text-[#475569] truncate max-w-[150px]">{batch.fileName}</td>
                <td className="py-3 px-4 text-right text-sm text-[#0f172a]">{batch.totalCount}</td>
                <td className="py-3 px-4 text-right text-sm text-[#0f172a]">{formatAmount(batch.totalAmount)}</td>
